...
class XyloSamna(Module):
    ...
    def _evolve_to_hidden(
        ...
        **kwargs,
        ...
        """
        Evolve a network on the Xylo HDK in single-step manual mode, injecting spikes into the hidden layer. For debug purposes only. Uses 'samna.xyloCore2.OperationMode.Manual' in samna.

        Sends a series of events to the Xylo HDK, evolves the network over the input events, and returns the output events produced during the input period.

        Args:
            input (np.ndarray): A raster ``(T, Nhidden)`` specifying for each bin the number of input events arising from the corresponding hidden neuron on Xylo, at the corresponding time point. Up to 31 input events can be sent per bin. These events will **replace** spiking events in the hidden layer. We recommend adding a dummy input layer, and treating the first hidden layer as the input layer.
            record (bool): Iff ``True``, record and return all internal state of the neurons and synapses on Xylo. Default: ``False``, do not record internal state.
            read_timeout (Optional[float]): Set an explicit read timeout for the entire simulation time. This should be sufficient for the simulation to complete, and for data to be returned. Default: ``None``, set a reasonable default timeout.
        ...
        Nin, Nhidden, Nout = self.shape
        self._configure_accel_time_mode(Nhidden, Nout, record)
        # - Ensure that input weights are zero
        self._config.input.weights = np.zeros_like(self._config.input.weights).tolist()
        ...
        # - Reset input spike registers, then write a single input event
        hdkutils.write_register(self._write_buffer, 0x11, 1)
        ...
        for timestep in tqdm(range(len(input))):
            # - Send input events for this time-step
            # hdkutils.send_immediate_input_spikes(self._write_buffer, input[timestep])
            hdkutils.write_memory(
                self._write_buffer, 0x0A540, len(input[timestep]), input[timestep]
            )
