        ...
        for batch, target in tqdm(test_dataloader):
            ...
            with torch.no_grad():
                ...
                for sample_data in batch:
                    ...
                    out_model, _, rec = model2(sample_data.astype(np.int16), record=False, record_power=True)
                    io_power = np.mean(rec['io_power'])
                    core_power = np.mean(rec['snn_core_power'])
                    print('io_power:',io_power,'snn_core_power:',core_power)
